  ...
  <!-- Leftbar -->
      ...
      <!-- RightBar -->
        <div class="place col-md-7 d-flex">
            <div class="card mt-5 box" style="width: 15rem; background: rgba(190, 195, 202, 0); cursor : pointer;
    border: none;"  v-for="item in data" :key="item.id">
...
export default {
  ...
  methods: {
    ...
    delet(){
      try {
        const response = axios.delete(process.env.VUE_APP_URL, this.formform)
        this.data = response.data  
      } catch (err) {
        console.log(err)
      }
