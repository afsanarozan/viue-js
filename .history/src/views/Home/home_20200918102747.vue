        ...
        <div class="place col-md-7 d-flex">
          <div class="cont" v-for="item in data" :key="item.id" >
           ...
           :images="showImage(item.images)" />
                ...
                <b-button variant="info ml-2" @click="delet(item.id)">Delete</b-button>   
           </div>
          </div>
  ...
  methods: {
    ...
    delet(id){
      try {
        ...
        const response = axios.delete(method: "DELETE",url: `${process.env.VUE_APP_URL}/api/product/${id}`, this.form})
        this.data = response.data  
      } catch (err) {
        console.log(err)
      }
